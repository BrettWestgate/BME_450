...
#define Extra 1                           //This doesn't matter it's only         
...
void loop() {
 ...
 //Motor.step(50, BACKWARD, DOUBLE);
  ...
  F=ServoBL.ReadInputs();       //The integers ("A","B",etc) are returned as 1 or 0 depending on if a state change has occured in the .ccp code 
    //Motor.off();
    delay(250);
    PlotPins();   //This operation is shown below and just allows us to see how the states are changing in response to the user input
  }
